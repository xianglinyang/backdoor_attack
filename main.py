...
from dataset import DataHandler, load_dataset, build_transform
...
def evaluate_badnets(data_loader_val_clean, data_loader_val_poisoned, model, batch_size, device):
    ta = eval(data_loader_val_clean, model, device, batch_size, print_perform=True)
    asr = eval(data_loader_val_poisoned, model, device, batch_size, print_perform=False)
    return {
...
def eval(data_loader, model, device, batch_size, print_perform=False):
    ...
    if print_perform:
        # TODO targets name
        print(classification_report(y_true.cpu(), y_predict.cpu(), target_names=np.arange(10).astype(str).tolist()))
...
def main():
    ...
    # TODO input_channel
    model = BadNet(input_channels=1, output_num=NB_CLASSES).to(device)
    criterion = torch.nn.CrossEntropyLoss()
    ...
    for epoch in range(EPOCHS):
        ...
        test_stats = evaluate_badnets(data_loader_val_clean, data_loader_val_poisoned, model, BATCH_SIZE, device)
